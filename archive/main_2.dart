...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<String> _fetchSummaryResponse(String transcript) async {
    if (_summarySpeechText != null) {
      return _summarySpeechText!;
    }
    ...
    try {
      final jsonData = await rootBundle.loadString('lib/stt/summary.json');
      ...
      final bulletPoints = payload['bullet_points'];
      if (bulletPoints is List) {
        final cleanBullets = bulletPoints
            .map((dynamic item) => item?.toString().trim())
            .whereType<String>()
            .where((element) => element.isNotEmpty)
            .toList();
        if (cleanBullets.isNotEmpty) {
          final bulletSentence = [
            'Key points are:',
            ...cleanBullets.asMap().entries.map(
                  (entry) => '${entry.key + 1}) ${entry.value}.',
                ),
          ].join(' ');
          sections.add(bulletSentence);
        }
      }

      final visual = payload['visual_description'];
      if (visual is String && visual.isNotEmpty) {
        sections.add('Visually: $visual');
      }

      _summarySpeechText = sections.join(' ');
      return _summarySpeechText!;
    } finally {
      if (mounted) {
        setState(() {
          ...
          child: Column(
            ...
            if (_image != null)
              ...
              VoiceAssistantWidget(
                fetchResponse: _fetchSummaryResponse,
                processingLabel: _isLoadingSummary ? 'Preparing summary…' : 'Processing…',
              ),
            ],
          ),
        ),
